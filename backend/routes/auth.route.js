...
router.post('/register', [
  ...
  body('password').matches(/^(?=.{11,}$)(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).*$/)
    .withMessage('Mật khẩu phải có ít nhất 11 ký tự, gồm chữ hoa, số và ký tự đặc biệt'),
...
router.post('/reset-password', [
  ...
  body('newPassword').matches(/^(?=.{11,}$)(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).*$/)
    .withMessage('Mật khẩu phải có ít nhất 11 ký tự, gồm chữ hoa, số và ký tự đặc biệt')
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
...
router.put('/change-password', protect, [
  ...
  body('newPassword').matches(/^(?=.{11,}$)(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).*$/)
    .withMessage('Mật khẩu mới phải có ít nhất 11 ký tự, gồm chữ hoa, số và ký tự đặc biệt')
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
